...
void put_line(void) {
	...
	if (current_line == line && last_filename == filename_text)
		return;
	...
	out_write(buffer, (TPP_Itos(buffer, (TPP(tint_t))(line + 1)) - buffer) * sizeof(char));
...
"tpp version " STR(TPP_API_VERSION) "/" STR(TPP_PREPROCESSOR_VERSION) " - New Tiny PreProcessor - "
	...
	(TPP(wstate_t))((TPPLexer_Current->l_warnings.w_curstate->ws_state[(wid) / (8 / TPP_WARNING_BITS)] >> \
			...
			for (iter = wgroup_names; *iter; ++iter) {
				fprintf(stderr, "-W%s%s\n",
				        getstate(iter - wgroup_names) == TPP(WSTATE_DISABLE) ? "no-"
				                                                             : "",
				        *iter);
			}
#undef getstate
...
static void pp_print(char const *buf, size_t bufsize) {
	if (outline_tokens == OUTLINE_MODE_TOK)
		out_write("[", sizeof(char));
	out_write(buf, bufsize * sizeof(char));
	switch (outline_tokens) {

	case OUTLINE_MODE_ZERO:
		out_write("\0", sizeof(char));
		is_at_linefeed = 1;
		break;
	case OUTLINE_MODE_TOK:
		out_write("]", sizeof(char));
		/* FALLTHRU */
	default:
		is_at_linefeed = 0;
...
static int pp_add_dep_filename(char const *filename, size_t filename_size) {
	...
	if (!dep_emit_dummy || dep_outfile == TPP_STREAM_INVALID)
		return 1;
...
static int pp_depcallback(struct TPPFile *file, int is_system_header) {
	...
	if (is_system_header && dep_nonsystem_only)
		return 1;
	if (!pp_add_dep_filename(file->f_name, file->f_namesize))
		return 0;
...
static struct TPPFile *TPPCALL
pp_depunknown(int mode, char *__restrict filename,
              size_t filename_size,
              struct TPPKeyword **pkeyword_entry) {
	(void)mode;
	(void)pkeyword_entry;
	if (!pp_add_dep_filename(filename, filename_size))
		TPPLexer_SetErr();
...
#define PRAGMA_COPYMASK           \
	...
	pp_print("#", 1);
	pp_print("pragma", 6);
	if (base_flags & TPPLEXER_FLAG_WANTSPACE)
		pp_print(" ", 1);
...
#define BACKWARDS(x) /* nothing */
	...
	while (argc && (argv[0][0] == '-' BACKWARDS(|| argv[0][0] == '/'))) {
		...
		if (!strcmp(arg, "-tok")
		    ...
		    BACKWARDS(|| !strcmp(arg, "-tokens"))) {
			outline_tokens = OUTLINE_MODE_TOK;
		} else if (!strcmp(arg, "fspc")) {
			TPPLexer_Current->l_flags |= TPPLEXER_FLAG_WANTSPACE;
		} else if (!strcmp(arg, "fno-spc")) {
			TPPLexer_Current->l_flags &= ~(TPPLEXER_FLAG_WANTSPACE);
		} else if (!strcmp(arg, "flf")) {
			TPPLexer_Current->l_flags |= TPPLEXER_FLAG_WANTLF;
		} else if (!strcmp(arg, "fno-lf")) {
			TPPLexer_Current->l_flags &= ~(TPPLEXER_FLAG_WANTLF);
		} else if (!strcmp(arg, "fcomments")) {
			TPPLexer_Current->l_flags |= TPPLEXER_FLAG_WANTCOMMENTS;
		} else if (!strcmp(arg, "fno-comments")) {
			TPPLexer_Current->l_flags &= ~(TPPLEXER_FLAG_WANTCOMMENTS);
		} else if (!strcmp(arg, "fmagiclf")) {
			no_magic_tokens = 0;
		} else if (!strcmp(arg, "fno-magiclf")) {
			no_magic_tokens = 1;
		} else if (!strcmp(arg, "fdecode")) {
			decode_tokens = 1;
		} else if (!strcmp(arg, "fno-decode")) {
			decode_tokens = 0;
		} else if (!strcmp(arg, "flongstring")) {
			TPPLexer_Current->l_flags &= ~(TPPLEXER_FLAG_TERMINATE_STRING_LF);
		} else if (!strcmp(arg, "fno-longstring")) {
			TPPLexer_Current->l_flags |= TPPLEXER_FLAG_TERMINATE_STRING_LF;
		} else if (!strcmp(arg, "funify-pragma")) {
			TPPLexer_Current->l_callbacks.c_parse_pragma = &reemit_pragma;
		} else if (!strcmp(arg, "fno-unify-pragma")) {
			TPPLexer_Current->l_callbacks.c_parse_pragma = NULL;
		} else if (!strcmp(arg, "fline")) {
			line_directives = 1;
		} else if (!strcmp(arg, "fno-line") BACKWARDS(|| !strcmp(arg, "no-line"))) {
			line_directives = 0;
		} else if (!strcmp(arg, "fcpp-line")) {
			line_directives = 2;
		} else if (!memcmp(arg, "ftabstop=", 9 * sizeof(char))) {
			TPPLexer_Current->l_tabsize = (size_t)atol(arg + 9);
		} else if (!strcmp(arg, "trigraphs")) {
			TPPLexer_EnableExtension(EXT_TRIGRAPHS);
		} else if (!strcmp(arg, "traditional") ||
		         !strcmp(arg, "traditional-cpp")) {
			...
			line_directives = 0;
		           ...
		           !strcmp(arg, "MM")) {
			/* Switch to dependency-only mode. */
			pp_mode = PPMODE_DEPENDENCY;
			/* Change the output file to stdout. */
			if (dep_outfile == TPP_STREAM_INVALID)
				dep_outfile = stdout_handle;
			/* Check if we`re supposed to include system-headeres. */
			dep_nonsystem_only = !strcmp(arg, "MM");
			...
			if (dep_outfile == TPP_STREAM_INVALID)
				dep_outfile = stdout_handle;
			/* Check if we`re supposed to include system-headeres. */
			dep_nonsystem_only = !strcmp(arg, "MMD");
		} else if (!strcmp(arg, "MG")) {
			/* Setup an unknown-file handler, assuming generated headers. */
			TPPLexer_Current->l_callbacks.c_unknown_file = &pp_depunknown;
			/* Disable file-not-found warnings (We`re assuming generated headers). */
			TPPLexer_SetWarning(TPP(W_FILE_NOT_FOUND), TPP(WSTATE_DISABLE));
			/* Switch to dependency-only mode. */
			pp_mode = PPMODE_DEPENDENCY;
		} else if (!strcmp(arg, "MP")) {
			dep_emit_dummy = 1; /* Emit a dummy target for every dependency. */
		} else if (!strcmp(arg, "MF")) {
			if (argc > 1) {
				 /* Close a previously opened dependency output stream. */
				if (dep_outfile != TPP_STREAM_INVALID && dep_outfile != stdout_handle)
					close_stream(dep_outfile);
				 dep_outfile = open_out_file(argv[1]);
				 ++argv;
				 --argc;
			}
		           ...
		           !strcmp(arg, "MQ")) {
			if (argc > 1) {
				dep_targetname = argv[1];
				++argv;
				--argc;
			}
			dep_escapetarget = !strcmp(arg, "MQ");
		} else if (!strcmp(arg, "o") BACKWARDS(|| !strcmp(arg, "-out"))) {
			if (argc > 1) {
		...
		} else if (!strcmp(arg, "-message-format=gcc")) {
...
BACKWARDS(add_def:)
			...
			if (!TPPLexer_Define(arg, strlen(arg), val, strlen(val),
			                     TPPLEXER_DEFINE_FLAG_BUILTIN))
				_exit(1);
		} else if (*arg == 'U') {
			if (!*++arg && argc > 1)
				arg = argv[1], ++argv, --argc;
			...
			if (add)
				TPPLexer_AddAssert(arg, strlen(arg), val, strlen(val));
			else {
			    TPPLexer_DelAssert(arg, strlen(arg), val, val ? strlen(val) : 0);
			}
		} else if (*arg == 'f') {
			int enable = 1;
			++arg; /* Set an extension. */
			if (!memcmp(arg, "no-", 3))
				arg += 3, enable = 0;
			if (!TPPLexer_SetExtension(arg, enable))
				goto noopt;
		} else if (*arg == 'W') {
			TPP(wstate_t)
			state = TPP(WSTATE_ERROR);
			++arg; /* Set an extension. */
			if (!memcmp(arg, "no-", 3))
				arg += 3, state = TPP(WSTATE_DISABLE);
			if (!TPPLexer_SetWarnings(arg, state))
				goto noopt;
		}
		BACKWARDS(else if (!memcmp(arg, "-Inc", 4)) {
			arg += 4;
			goto add_inc;
		})
		BACKWARDS(else if (!memcmp(arg, "-Def", 4)) {
			arg += 4;
			goto add_def;
		})
		else {
noopt:
	...
	if (output_filename && strcmp(output_filename, "-")) {
		TPP(stream_t)
		newout = open_out_file(output_filename);
		if (dep_outfile == stdout_handle)
			dep_outfile = newout;
#ifdef _WIN32
...
use_infile:
	...
	if (!pp_depfirst(infile))
		goto err1;
	base_flags = TPPLexer_Current->l_flags;
	switch (pp_mode) {

	case PPMODE_DEPENDENCY:
		pp_deponly();
		break;
